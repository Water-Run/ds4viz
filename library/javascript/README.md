...
在浏览器环境中，由于沙箱执行的限制，`ds4viz` 无法通过调用栈自动获取准确的代码行号。为解决此问题，库支持通过参数注入行号信息。

##### 使用方式

在调用数据结构方法时，可以在最后一个参数位置传入 `{ __line: 行号 }` 对象：

```javascript
s.push(10, { __line: 5 });      // 带参数的方法
s.pop({ __line: 6 });           // 无参数的方法
```

如果未提供行号参数，库会尝试通过调用栈自动获取行号（在 Node.js 环境或支持的浏览器中）。

##### 自动注入

如果你使用代码预处理器或构建工具，可以自动将源代码中的方法调用转换为带行号的形式：
...
这确保了在任何执行环境下都能获得准确的代码位置信息，便于可视化时定位到对应的源代码行。
