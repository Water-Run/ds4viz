...
<template>
  <Transition name="error-banner">
    <div v-if="message.length > 0" class="error-banner" role="alert">
      <MaterialIcon name="error" class="error-banner__icon" />
      <span class="error-banner__text">{{ message }}</span>
      <button class="error-banner__close" aria-label="关闭" @click="$emit('dismiss')">
...
.error-banner__close :deep(.material-icon) {
  width: 16px;
  height: 16px;
}
