...
const handleLogin = async (): Promise<void> => {
  ...
  try {
    ...
    const redirect =
      typeof route.query.redirect === 'string' ? route.query.redirect : '/playground'
      ...
      <section class="auth-body entrance" :style="{ '--d': '250ms' }">
        <form class="auth-form" novalidate @submit.prevent="handleLogin">
          <div class="auth-form__header">
            <h1 class="auth-form__title">登录</h1>
          </div>
          ...
          <div class="auth-form__footer">
            <router-link :to="{ name: 'register' }" class="link-btn">创建账号</router-link>
            <button
              type="submit"
              class="primary-btn"
...
.auth-brand__slogan {
  margin: 0;
  font-size: 15px; color: $text-muted; line-height: 1.6;
}
