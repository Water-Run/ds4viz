 ...
 * @returns 子节点数组
 */
const getChildren = (node: DocNode): DocNode[] => node.children ?? []
...
const docTree = computed<DocNode[]>(() => [
  {
    ...
    title: '概览',
    content:
      'ds4viz（data-structure-for-visualization）是一个面向教学场景的基础数据结构可视化框架。核心架构通过提供对应语言库，将代码中的结构操作过程记录为可回放、可追踪、可复现的状态序列，并存储在通用的 TOML IR 中。由于各层解耦的设计，ds4viz 支持多种渲染器。用户可以在单个页面完成从编写代码到观察结构变化的全过程。ds4viz 遵循 GPL v3 协议开放源代码。',
    children: [
      {
        id: 'intro-principle',
        title: '基础原理',
        content:
          'ds4viz 采用 代码 → 中间语言 → 渲染 三层解耦架构。第一层是语言库：用户在 Python、C 等语言中以指定语法调用 ds4viz API，每次操作都会记录状态变化。第二层是中间语言 IR：所有语言统一产出 .toml 文件，该文件描述一次完整 trace，渲染器不需要读取源码，只需读取 TOML 即可复原整个过程。第三层是渲染器：按 object.kind 解析结构类型，按 states 和 steps 进行展示，同一份 TOML 可在不同渲染端复用。一类特定的、不设计用于交互的渲染器在 ds4viz 中被称为编译器，例如输出到 PowerPoint 演示文稿等。',
      },
    ],
  },
  {
    ...
    title: '在线服务',
    content:
      'ds4viz 提供在线服务，这是使用 ds4viz 最方便的方式。当前页面即 ds4viz 提供的网页端，使用 Vue SPA + FastAPI + PostgreSQL 技术栈，将代码编辑、远程执行、TOML IR 解析、可视化渲染整合到同一工作流中。在线服务需要登录才可访问，需要先注册帐号。',
    children: [
      {
        id: 'online-pages',
        title: '页面架构',
        content:
          '进入系统后，左侧为导航栏，可折叠。页面包括：关于页、可视化和编辑器页、模板库页、文档页、个人主页。个人主页支持头像更新、密码修改、统计数据查看、收藏模板与执行记录管理。',
      },
      {
        id: 'online-editor',
        title: '编辑器',
        content:
          '编辑器页采用左右分栏。左侧为可视化渲染器，支持前进、后退、首尾跳转、自动播放、拖动画布和缩放。右侧为 Monaco 编辑区域，上方下拉框切换语言（仅 Python / C），支持基础语法高亮与补全。运行后后端返回 TOML IR，前端解析并刷新可视化。支持下载与上传 TOML；上传本地 TOML 时，代码行高亮无法对应。',
      },
      {
        id: 'online-renderer',
        title: '可视化渲染器',
        content:
          '渲染器按 object.kind 识别结构类型并绘制，渲染过程基于 states 完整快照，steps 用于展示操作名、参数、返回值和源码行号。支持自动播放或逐帧步进。可在可视化面板调整运行时配置：显示元数据、平滑动画衔接、联动代码高亮、自适应画布、变更强化、控制台调试、跳过初始页、生成后自动播放、播放间隔。',
      },
      {
        id: 'online-templates',
        title: '模板系统',
        content:
          '在线服务提供模板系统，支持分类筛选、搜索、收藏。模板详情支持语言切换并可一键导入编辑器继续运行。',
      },
    ],
  },
  {
    ...
    title: '语法',
    content:
      'ds4viz 当前支持 Python、C 两种语言，遵循一致的语义设计：创建结构对象，在受控上下文中执行操作，自动生成 trace.toml，成功写入 [result]，失败写入 [error]。因需要确保 TOML IR 始终写入，Python 使用 with，C 使用结构作用域宏。值类型仅支持 integer、float、string、boolean。',
    children: [
      {
        id: 'syntax-config',
        title: '全局配置',
...
int main(void) {
  ...
  return 0;
      ...
      },
      {
        ...
        title: '上下文管理',
        content:
          '使用上下文管理器确保无论成功或失败，都会生成 trace.toml。成功时写入 [result]，异常时写入 [error]。注意不要在 C 的结构作用域中使用 return 或 goto 跳出。',
        codeBlocks: {
          python: `import ds4viz as dv
...
int main(void) {
  ...
  },
  {
    ...
    title: '自部署服务',
    content:
      'ds4viz 是开源的，可自行部署在线服务。需要配置运行环境，并安装当前支持语言对应的 ds4viz 库。详情参阅 GitHub 上的在线服务部署文档。',
    children: [
      {
        id: 'deploy-install',
        title: '库安装',
        content: '按需安装 Python / C 语言支持。',
        codeBlocks: {
      ...
      <aside class="docs-page__toc">
        <div v-for="section in docTree" :key="section.id" class="toc-section">
          ...
          <div v-if="expanded[section.id]" class="toc-section__children">
            <button v-for="child in getChildren(section)" :key="child.id" class="toc-section__item"
