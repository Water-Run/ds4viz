/*
 * 带权图数据结构实现
 * 提供带权图的可视化追踪功能，支持有向和无向模式，边带有权重
 * 
 * @author: WaterRun
 * @file: src/Structures/GraphWeighted.cs
 * @date: 2026-02-06
 */
...
/// <summary>带权图数据结构，提供带权边连接的节点存储与追踪功能。</summary>
/// <remarks>
/// 不变量：不允许自环和重边；无向模式下边存储为 from 小于 to。
/// 线程安全：不保证线程安全。
/// 副作用：所有操作方法都会记录追踪步骤。
/// </remarks>
public sealed class GraphWeighted : BaseStructure
{
    /// <summary>节点存储字典，键为节点 Id，值为标签。</summary>
    private readonly Dictionary<int, string> _nodes = [];

    /// <summary>带权边字典，键为 (from, to) 元组，值为权重。</summary>
    private readonly Dictionary<(int From, int To), double> _edges = [];

    /// <summary>是否为有向图。</summary>
    ...
    internal GraphWeighted(string label = "graph", bool directed = false, string? output = null)
        : base("graph_weighted", label, output)
    {
        _isDirected = directed;
    }

    /// <summary>构建带权图的当前状态数据。</summary>
    ...
    public void AddEdge(int fromId, int toId, double weight, [CallerLineNumber] int callerLine = 0)
    {
        ...
        var edge = NormalizeEdge(fromId, toId);
        if (_edges.ContainsKey(edge))
        {
            RaiseError($"Edge already exists: ({edge.From}, {edge.To})", callerLine);
        }
        ...
        _session.AddStep(
            ...
            before,
            after,
            new Dictionary<string, object> { ["from"] = fromId, ["to"] = toId, ["weight"] = weight },
            line: callerLine);
    }
    ...
    public void RemoveEdge(int fromId, int toId, [CallerLineNumber] int callerLine = 0)
    {
        ...
        if (!_edges.ContainsKey(edge))
        {
            RaiseError($"Edge not found: ({edge.From}, {edge.To})", callerLine);
        }
    ...
    /// <summary>更新边的权重。</summary>
    /// <param name="fromId">起始节点 Id。</param>
    /// <param name="toId">终止节点 Id。</param>
    /// <param name="weight">新的权重值。</param>
    /// <param name="callerLine">调用者行号。</param>
    /// <exception cref="StructureException">当边不存在时抛出。</exception>
    public void UpdateWeight(int fromId, int toId, double weight, [CallerLineNumber] int callerLine = 0)
    {
        var edge = NormalizeEdge(fromId, toId);
        if (!_edges.ContainsKey(edge))
        {
    ...
    /// <summary>规范化边，无向图时确保 from 小于 to。</summary>
    /// <param name="fromId">起始节点 Id。</param>
    /// <param name="toId">终止节点 Id。</param>
    /// <returns>规范化后的边元组。</returns>
    private (int From, int To) NormalizeEdge(int fromId, int toId)
    {
        if (_isDirected)
        {
            return (fromId, toId);
