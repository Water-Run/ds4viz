/*
 * 无向图数据结构实现
 * 提供无向图的可视化追踪功能，支持节点和边的增删操作
 * 
 * @author: WaterRun
 * @file: src/Structures/GraphUndirected.cs
 * @date: 2026-02-06
 */
...
/// <summary>无向图数据结构，提供无向边连接的节点存储与追踪功能。</summary>
/// <remarks>
/// 不变量：边以 (from, to) 形式存储且 from 小于 to；不允许自环和重边。
/// 线程安全：不保证线程安全。
/// 副作用：所有操作方法都会记录追踪步骤。
/// </remarks>
public sealed class GraphUndirected : BaseStructure
{
    /// <summary>节点存储字典，键为节点 Id，值为标签。</summary>
    private readonly Dictionary<int, string> _nodes = [];

    /// <summary>边集合，存储为 (from, to) 元组且 from 小于 to。</summary>
    ...
    internal GraphUndirected(string label = "graph", string? output = null)
        : base("graph_undirected", label, output)
    {
    }

    /// <summary>构建无向图的当前状态数据。</summary>
    ...
    /// <summary>添加无向边，内部自动规范化为 fromId 小于 toId。</summary>
    ...
    /// <summary>规范化无向边，确保 from 小于 to。</summary>
    /// <param name="fromId">起始节点 Id。</param>
    /// <param name="toId">终止节点 Id。</param>
    /// <returns>规范化后的边元组。</returns>
    private static (int From, int To) NormalizeEdge(int fromId, int toId)
    {
